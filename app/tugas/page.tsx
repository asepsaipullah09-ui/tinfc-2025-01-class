...
export default function TugasPage() {
  ...
  const filteredTugas = useMemo(() => {
    return tugas.filter((t) => {
      ...
      const matchJudul = t.judul.toLowerCase().includes(searchJudul.toLowerCase());
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <h1 className="text-2xl font-bold text-slate-800">Upload Tugas Mahasiswa</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
          <h2 className="text-lg font-semibold text-slate-800">Cari & Filter</h2>
          {hasActiveFilters && (
            <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label className="block text-sm font-medium text-slate-600 mb-1">Cari Nama</label>
            <input
              type="text"
              value={searchNama}
          ...
          <div>
            <label className="block text-sm font-medium text-slate-600 mb-1">Cari NIM</label>
            <input
              type="text"
              value={searchNim}
          ...
          <div>
            <label className="block text-sm font-medium text-slate-600 mb-1">Cari Judul Tugas</label>
            <input
              type="text"
              value={searchJudul}
        ...
        {hasActiveFilters && (
          <div className="mt-4 p-3 bg-purple-50 rounded-lg">
            <p className="text-sm text-purple-700">
              Menampilkan <span className="font-bold">{filteredTugas.length}</span> dari <span className="font-bold">{tugas.length}</span> data tugas
            </p>
          </div>
        )}
      </div>
        ...
        <table className="w-full">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className="px-6 py-4 text-left text-sm font-semibold text-slate-700">Judul Tugas</th>
        ...
        {filteredTugas.map((t) => (
          <div key={t.id} className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
            <div className="space-y-3">
              ...
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-wide">Judul</p>
